...
async fn main() {
    ...
    info!("Starting Metro CCDC Individual Scoreboard v{}", version);
    ...
    let app = Router::new()
        ...
        .layer(cors)
        .with_state(state);
    // 8001 by default so it can run alongside the team scoreboard on 8000.
    let port = std::env::var("SB_PORT").unwrap_or_else(|_| "8001".to_string());
