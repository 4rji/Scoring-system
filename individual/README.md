...
Copia del scoreboard de `metrosco/` adaptada para puntuar **participantes individuales**:
cada estudiante tiene sus propias IPs y su propia lista de servicios, y el dashboard
muestra una fila por participante con el estado de cada uno de sus servicios:

```
Connor   [AD] [DNS] [FTP] [WEB]   uptime 97.5%
Maria    [DNS] [SSH] [WEB]        uptime 100%
```

Verde = arriba, rojo = caído, gris = todavía no se ha verificado. Cada servicio muestra
su % de uptime (checks exitosos / checks totales). Los injects funcionan igual que en
`metrosco/`, por participante.

## Uso rápido
1. Define el catálogo de servicios en `resources/services.yaml` (nombre → comando del checker).
2. Define los participantes en `resources/teams.yaml`. La variable `SERVICES` dice qué
   servicios del catálogo se le verifican a cada uno (si se omite, se verifican todos):
   ```yaml
   Connor:
     SERVICES: "AD,DNS,FTP,WEB"
     IP: "10.0.10.10"          # opcional: ping en "Participant hosts"
     AD_HOST: "10.0.10.10"
     DNS_SERVER: "10.0.10.10"
     DNS_RECORD: "connor.lab.local"
     FTP_HOST: "10.0.10.11"
     FTP_USER: "user"
     FTP_PASS: "pass"
     WEB_URL: "http://10.0.10.12"
   ```
3. `cd individual && cargo run -r` y abre http://localhost:8001 (puerto 8001 por defecto
   para poder correrlo junto a `metrosco/` en 8000; cámbialo con `SB_PORT`).
4. Los injects se configuran en `resources/injects.csv`, igual que en `metrosco/`.

`SERVICES` también se puede editar en caliente desde `/admin` (variables del equipo).
Tras cambiar el cliente React, reconstruye con `just buildspa`.
...
4. `cargo run -r`
You should now have the scoreboard up and running at http://localhost:8001
...
- SB_PORT: What port the scoreboard will listen on. 8001 by default.
